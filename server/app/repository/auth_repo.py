...
class JWTBearer(HTTPBearer):
    ...
    async def __call__(self, request: Request):
        ...
        if credentials:
            if not credentials.scheme == "Bearer":
                raise HTTPException(status_code=403, detail={"status": "Forbidden", "message": "Invalid authentication scheme."})
    ...
    def verify_jwt(jwt_token: str):
        return True if jwt.decode_token(jwt_token, SECRET_KEY, algorithms=[ALGORITHM]) is not None else False
